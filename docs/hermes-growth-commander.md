...
3. If no eligible work remains, run kgrowth weekly/analyze through kdeck, sync improvement goals, and enable only implemented executable jobs.
4. If kgrowth proposes an improvement whose function is not implemented, do not fake success. Record an event that implementation is needed.
5. If implementation is needed, create a short Codex/OpenClaw task instruction that names the owning repository and the exact expected function or file, then stop.
6. Never treat RQ enqueue success as business success.
7. Do not edit secrets, print tokens, or commit credentials.
8. Do not put app-specific job modules in rqdb4ai.
9. Keep replies short and operational.
