...
<script>
	...
	function renderController(data){
	  ...
	  const nextText = summary.next_goal_name
	    ? `${summary.next_goal_name} / ${fmtIsoTime(summary.next_eligible_at)}`
	    : '次回候補なし';
	  ...
	  goalList.innerHTML = goals.map(goal => {
	    ...
	    const nextWhen = goal.next_eligible_at ? fmtIsoTime(goal.next_eligible_at) : '-';
